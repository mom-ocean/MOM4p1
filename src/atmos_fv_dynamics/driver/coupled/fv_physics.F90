...
!-----------------------------------------------------------------------
character(len=128) :: version = '$Id: fv_physics.F90,v 17.0.2.2 2009/12/09 23:05:11 wfc Exp $'
...
!physics window
    ...
    allocate( p_full(nx_win,ny_win,nlev) )
    allocate( z_full(nx_win,ny_win,nlev) )
    allocate( p_half(nx_win,ny_win,nlev+1) )
    allocate( z_half(nx_win,ny_win,nlev+1) )
!MPP clocks
...
!isw,iew,jsw,jew are in global index space
    do jsw = beglat,endlat,ny_win
       ...
       do isw = beglon,endlon,nx_win
          iew = isw + nx_win - 1
          call compute_p_z( isw, jsw, nx_win, ny_win )
