...
use ocean_parameters_mod,only: missing_value, rho0r, rho0
...
integer :: id_neutral_rho_overexch=-1
integer :: id_wdian_rho_overexch  =-1

integer, dimension(:), allocatable :: id_overexch


character(len=128) :: version=&
       '=>Using: ocean_overexchange.f90 ($Id: ocean_overexchange.F90,v 16.0.108.1 2009/10/10 00:42:43 nnz Exp $)'
...
! </DESCRIPTION>
!
  subroutine ocean_overexchange_init(Grid, Domain, Time, T_prog, Ocean_options, &
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    id_neutral_rho_overexch = register_diag_field ('ocean_model', 'neutral_rho_overexch', &
                        Grd%tracer_axes(1:3), Time%model_time,                            &
                        'update of neutral density from overexchange scheme',             &
                        'rho*rho_dz/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
    id_wdian_rho_overexch = register_diag_field ('ocean_model', 'wdian_rho_overexch',   &
                            Grd%tracer_axes(1:3), Time%model_time,                      &
                            'dianeutral velocity component due to overexchange scheme', &
...
subroutine overexchange (Time, Thickness, T_prog, Dens, index_temp, index_salt)
  ...
  do nt=1,num_prog_tracers 
     ...
     if(id_neutral_rho_overexch > 0 .or. id_wdian_rho_overexch > 0) then 
         if(nt==index_temp) then 
             wrk1_v(:,:,:,1) = 0.0
             do k=1,nk
                do j=jsc,jec   
  ...
  ! send diagnostics for update of neutral density from overexchange scheme 
  if (id_neutral_rho_overexch > 0) then 
     used = send_data (id_neutral_rho_overexch,                                                &
                       Dens%drhodT(:,:,:)*wrk1_v(:,:,:,1)+Dens%drhodS(:,:,:)*wrk1_v(:,:,:,2) , &
                       Time%model_time,rmask=Grd%tmask(:,:,:),                                 &
                       is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif 
  if (id_wdian_rho_overexch > 0) then 
      wrk1(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
            do i=isc,iec
         ...
         enddo
      enddo
      used = send_data (id_wdian_rho_overexch, wrk1(:,:,:), &
           Time%model_time,rmask=Grd%tmask(:,:,:),          &
           is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
  ...
  if (debug_this_module) then
      ...
      write (stdoutunit, *) 'chksum for rho_dzt_ex  = ',mpp_chksum(rho_dzt_ex(isc:iec,jsc:jec,:))
      ...
      do m=1,4
         write (stdoutunit, *)    'chksum for kup_ex          = ',mpp_chksum(kup_ex(isc:iec,jsc:jec,m))
         do n=1,overexch_npts
            write (stdoutunit, *) 'chksum for kdw_ex          = ',mpp_chksum(kdw_ex(isc:iec,jsc:jec,n,m))
            write (stdoutunit, *) 'chksum for overexch_flux   = ',mpp_chksum(overexch_flux(isc:iec,jsc:jec,n,m))
         enddo
      enddo
