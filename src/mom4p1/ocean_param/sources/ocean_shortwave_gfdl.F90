...
!  mom4p0. The new subroutine removes some confusing and unnecessary
...
!  mom4p0 for use with chlorophyll data. This scheme is NOT available
...
!  of legacy, as this was done in mom4p0 versions. The default is 
...
use constants_mod,            only: rho_cp, epsln, c2dbars
...
character(len=128)  :: version='$Id: ocean_shortwave_gfdl.F90,v 1.1.4.21.110.1 2009/10/10 00:42:50 nnz Exp $'
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    if(optics_morel_antoine) then 
        ...
        if(sw_morel_fixed_depths) then
            ...
            write(stdoutunit,'(a)') &
                 '             setting sw_morel_fixed_depths=.false.  However, to reproduce mom4p0'
            write(stdoutunit,'(a)') &
...
!
! <DESCRIPTION>
! As in sw_morel, but uses the mom4p0 algorithm to re-compute a k-level. 
