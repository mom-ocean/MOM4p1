...
use ocean_parameters_mod,      only: rho0r, rho0, m3toSv
...
character(len=128) :: version=&
     '$Id: ocean_nphysicsB.F90,v 1.1.2.27.12.7.18.2.18.1.32.1.4.1 2009/10/21 20:07:27 smg Exp $'
...
namelist /ocean_nphysicsB_nml/                            &
          ...
          dm_taper, gkw_taper, transport_units
...
! </DESCRIPTION>
!
subroutine ocean_nphysicsB_init(Grid, Domain, Time, Time_steps, Thickness, T_prog,&
  ...
  if(use_this_module) then 
    call mpp_error(NOTE, &
    '==> from ocean_nphysicsB_mod: USING ocean_nphysicsB_mod.')
    write(stdoutunit,'(1x,a)')    &
    '==> Note from ocean_nphysicsB_mod: USING ocean_nphysicsB.'
  else 
    call mpp_error(NOTE, &
    '==> from ocean_nphysicsB_mod: NOT using ocean_nphysicsB_mod.')
    write(stdoutunit,'(1x,a)')    &
    '==> Note from ocean_nphysicsB_mod: NOT using ocean_nphysicsB.'
    return
  endif 
  ...
  if(debug_this_module) then 
    write(stdoutunit,'(a)') &
     '==>Note: running ocean_nphysicsB_mod with debug_this_module=.true.'  
  endif 
  ...
  if(use_gm_skew) then
    gm_skew=1.0
    write(stdoutunit,'(1x,a)')' ==> Note from ocean_nphysicsB_init: use_gm_skew=.true. so will use GM-skewsion.' 
  else
    gm_skew=0.0
    write(stdoutunit,'(1x,a)')' ==> Note from ocean_nphysicsB_init: use_gm_skew=.false. so will NOT use GM-skewsion.' 
  endif

  if(dm_taper .and. .not. gkw_taper) then
      write(stdoutunit,'(1x,a)') &
      ' ==>Note from ocean_nphysicsB_mod: dm_taper=.true. Will use the tanh scheme ' 
  ...
  if(gkw_taper .and. .not. dm_taper) then
      write(stdoutunit,'(1x,a)') &
      ' ==>Note from ocean_nphysicsB_mod: gkw_taper=.true. Will use the quadratic scheme' 
      write(stdoutunit,'(7x,a)') &
  ...
  if(neutral_physics_limit) then
          write(stdoutunit,'(1x,a)') &
      ' ==>Note from ocean_nphysicsB_mod: neutral_physics_limit=.true.' 
          write(stdoutunit,'(7x,a)') &
  ...
  do j=jsd,jed
     do i=isd,ied
        ksurf_blayer(i,j)        = 1
        surf_turb_thick(i,j)     = Grd%tmask(i,j,1)*surf_turb_thick_min
        surf_trans_thick(i,j)    = 0.0
        depth_blayer_base(i,j)   = surf_turb_thick(i,j) + surf_trans_thick(i,j)
        full_turb_column(i,j)    = 0.0
        slope_blayer_base(i,j)   = 0.0
     enddo
  enddo
  ...
  if(.NOT.file_exist('INPUT/ocean_neutralB.res.nc')) then
      ...
      write(stdoutunit,*)' ' 
      write(stdoutunit,*) 'From ocean_nphysicsB_init: initial neutral checksums'
...
subroutine nphysicsB (Time, Thickness, Dens, rho, T_prog, &
  ...
  do j=jsc,jec
     do i=isc,iec
        surf_bdlthick(i,j) = surf_blthick(i,j)
     enddo
  enddo
...
subroutine neutral_blayer(Time, Thickness)
  ...
  do k=1,nk
     do j=jsc,jec
        do i=isc,iec 
           wrk1(i,j,k) = max( abs(tensor_31(i,j,k,0,0)),abs(tensor_31(i,j,k,0,1)), &
                              abs(tensor_31(i,j,k,1,0)),abs(tensor_31(i,j,k,1,1)), & 
                              abs(tensor_32(i,j,k,0,0)),abs(tensor_32(i,j,k,0,1)), &
                              abs(tensor_32(i,j,k,1,0)),abs(tensor_32(i,j,k,1,1)) )
        enddo
     enddo
  enddo
  ...
  if (id_surf_turb_thick >  0) then 
       used = send_data (id_surf_turb_thick, surf_turb_thick(:,:), &
              Time%model_time, rmask=Grd%tmask(:,:,1),             &
              is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif 
  if (id_surf_trans_thick >  0) then 
       used = send_data (id_surf_trans_thick, surf_trans_thick(:,:), &
              Time%model_time, rmask=Grd%tmask(:,:,1),               &
              is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif 
  ...
  if (id_full_turb_column >  0) then 
      used = send_data (id_full_turb_column, full_turb_column(:,:), &
           Time%model_time, rmask=Grd%tmask(:,:,1),                 &
           is_in=isc, js_in=jsc, ie_in=iec, je_in=jec)
  endif
...
subroutine neutral_chksums
  ...
  write (stdoutunit, *) &
  'chksum for surf_turb_thick  = ',mpp_chksum(surf_turb_thick(isc:iec,jsc:jec)*Grd%tmask(isc:iec,jsc:jec,1))
  write (stdoutunit, *) &
  'chksum for surf_trans_thick  = ',mpp_chksum(surf_trans_thick(isc:iec,jsc:jec)*Grd%tmask(isc:iec,jsc:jec,1))
...
subroutine ocean_nphysicsB_end(Time)
  ...
  write(stdoutunit,*)' ' 
  write(stdoutunit,*) 'From ocean_nphysicsB_end: ending neutral checksums'
  call write_timestamp(Time%model_time)
  call neutral_chksums
