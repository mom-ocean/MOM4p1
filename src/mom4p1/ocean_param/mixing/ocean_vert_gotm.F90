...
!
! mom4p1 is distributed with the basic routines from the 4.0
...
!
! In the hydro model (i.e., mom4), buoyancy fluxes are the 
...
! momentum mixing in mom4.  This information then enters GOTM 
...
use ocean_parameters_mod, only: missing_value, onesixth, rho0r
...
character(len=256) :: version=&
     '$Id: ocean_vert_gotm.F90,v 16.0.2.3.6.1.2.1.52.1 2009/10/10 00:42:33 nnz Exp $'
...
!
! <DESCRIPTION>
! Initialization for the mom4 wrapper to the GOTM vertical mixing scheme.
!
! For restarts: 
!
...
subroutine ocean_vert_gotm_init (Grid, Domain, Time, Time_steps, T_prog, obc, debug)
  ...
  id_gotm_ss = register_diag_field ('ocean_model', 'gotm_ss',                        &
       Grd%tracer_axes_wt(1:3), Time%model_time, 'squared vert shear given to GOTM', &
       '1/s^2', missing_value=missing_value, range=(/-1e18,1e18/))

  id_diff_cbt_gotm = register_diag_field('ocean_model','diff_cbt_gotm',Grd%tracer_axes_wt(1:3), &
       Time%model_time, 'vert diffusivity from GOTM', 'm^2/sec',                                &
       missing_value = missing_value, range=(/-1.e5,1.e5/))

  id_visc_cbu_gotm = register_diag_field('ocean_model','visc_cbu_gotm',Grd%vel_axes_wu(1:3), &
...
! </DESCRIPTION>
!
subroutine vert_mix_gotm (Time, Thickness, Velocity, T_prog, Dens, visc_cbu, diff_cbt)
  ...
  if ( .not. module_is_initialized ) then 
     call mpp_error(FATAL,&
      '==>Error from ocean_vert_gotm: module must be initialized')
  endif 
      ...
      do j=jsc,jec
         do i=isc,iec
            ...
            if(Grd%tmask(i,j,1) > 0.0) then 
                ...
                smf1_active = (Velocity%smf(i,j,1)   + Velocity%smf(i-1,j,1) + &
                               Velocity%smf(i,j-1,1) + Velocity%smf(i-1,j-1,1))/active_cells
                smf2_active = (Velocity%smf(i,j,2)   + Velocity%smf(i-1,j,2) + &
                               Velocity%smf(i,j-1,2) + Velocity%smf(i-1,j-1,2))/active_cells
