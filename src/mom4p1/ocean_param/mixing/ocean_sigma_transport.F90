...
! and has many problems.  It is retained in mom4p1 for 
...
! and it retained in mom4p1 only for research purposes.  It 
...
integer :: id_neutral_rho_sigma=-1
...
character(len=128) :: version=&
     '$Id: ocean_sigma_transport.F90,v 16.0.42.1.54.1 2009/10/10 00:42:29 nnz Exp $'
...
! </DESCRIPTION>
!
subroutine ocean_sigma_transport_init(Grid, Domain, Time, T_prog, Ocean_options, &
  ...
  ! diagnostics for neutral density  
  id_neutral_rho_sigma = register_diag_field ('ocean_model', 'neutral_rho_sigma', &
                        Grd%tracer_axes(1:3), Time%model_time,                    &
  ...
  id_wdian_rho_sigma = register_diag_field ('ocean_model', 'wdian_rho_sigma',  &
                       Grd%tracer_axes(1:3), Time%model_time,                  &
                       'dianeutral velocity component due to sigma transport', &
...
! has fundamental problems.  It is retained in mom4p1
...
subroutine sigma_transport (Time, Thickness, Dens, T_prog, Adv_vel, bott_blthick)
  ...
  do j=jsd,jed-1
     do i=isd,ied-1
        ...
        if(tmask_sigma(i,j)==1.0) then 
            ...
            salt0   = tracer_sigma(i,j,  index_salt)
            tempip1 = tracer_sigma(i+1,j,index_temp)
            saltip1 = tracer_sigma(i+1,j,index_salt) 
            tempjp1 = tracer_sigma(i,j+1,index_temp)
            saltjp1 = tracer_sigma(i,j+1,index_salt)
  ...
  do n=1,num_prog_tracers
     ...
     ! minus sign accounts for mom4 sign convention for diffusive flux.  
         ...
         do j=jsc,jec
            do i=isc,iec
                ...
                *(tchg_diff(i,j,index_temp)+tchg_adv(i,j,index_temp)+tchg_smooth(i,j,index_temp)) &
                            +Dens%drhodS(i,j,k)*wrk1(i,j,k)                                       &
                *(tchg_diff(i,j,index_salt)+tchg_adv(i,j,index_salt)+tchg_smooth(i,j,index_salt)) 
               wrk2(i,j,k) = wrk2(i,j,k)/(epsln+temporary)
            enddo
         enddo
      enddo
      used = send_data (id_wdian_rho_sigma, wrk2(:,:,:), &
             Time%model_time,rmask=Grd%tmask(:,:,:),     &
             is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
  endif
