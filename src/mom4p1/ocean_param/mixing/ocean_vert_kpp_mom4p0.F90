...
! and so is NOT generally recommended.  It remains part of mom4p1 for 
! legacy purposes. 
!
...
use ocean_parameters_mod, only: GEOPOTENTIAL, cp_ocean, rho0, rho0r, missing_value
...
character(len=256) :: version=&
     '$Id: ocean_vert_kpp_mom4p0.F90,v 16.0.2.2.40.1.12.1 2009/10/10 00:42:35 nnz Exp $'
...
! </DESCRIPTION>
!
subroutine ocean_vert_kpp_mom4p0_init (Grid, Domain, Time, Time_steps, T_prog, T_diag, ver_coordinate)
...
!-----------------------------------------------------------------------
  ...
  id_diff_cbt_int = register_diag_field('ocean_model','diff_cbt_int',Grd%tracer_axes_wt(1:3), &
       Time%model_time, 'vert diffusivity from kpp internal tide mixing', 'm^2/sec',          &
       missing_value = missing_value, range=(/-1.e5,1.e5/))

  id_visc_cbt_int = register_diag_field('ocean_model','visc_cbt_int',Grd%tracer_axes_wt(1:3), &
       Time%model_time, 'vert viscosity from kpp internal tide mixing', 'm^2/sec',&
       missing_value = missing_value, range=(/-1.e5,1.e5/))

  id_diff_cbt_coast = register_diag_field('ocean_model','diff_cbt_coast',Grd%tracer_axes_wt(1:3), &
       Time%model_time, 'vert diffusivity from kpp coastal tide mixing', 'm^2/sec',               &
       missing_value = missing_value, range=(/-1.e5,1.e5/))

  id_visc_cbt_coast = register_diag_field('ocean_model','visc_cbt_coast',Grd%tracer_axes_wt(1:3), &
  ...
  id_bf_int_tide    = register_diag_field('ocean_model','bf_int_tide',Grd%tracer_axes_wt(1:3), &
