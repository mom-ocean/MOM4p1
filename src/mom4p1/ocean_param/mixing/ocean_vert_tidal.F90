...
!  This value is needed in order to regularize the diffisuvity computed
...
! </DESCRIPTION>
  ...
  stdoutunit=stdout();stdlogunit=stdlog() 
    ...
    id_energy_flux = register_diag_field ('ocean_model', 'energy_flux',                 &
         Grid%tracer_axes(1:2), Time%model_time, 'energy flux out of barotropic tides', &
...
!
  ...
  if(use_wave_dissipation)  call vert_mix_wave(Time, Thickness, diff_cbt, visc_cbu, diff_cbt_wave)
  if(use_drag_dissipation)  call vert_mix_drag(Time, Thickness, diff_cbt, visc_cbu, diff_cbt_drag)
  ...
  if(id_power_diss_wave > 0 .or. id_power_diss_drag > 0) then 
      wrk1(:,:,:) = 0.0
      wrk2(:,:,:) = 0.0
      do k=1,nk
         do j=jsc,jec
...
! </DESCRIPTION>
!
  subroutine vert_mix_wave(Time, Thickness, diff_cbt, visc_cbu, diff_cbt_wave)
