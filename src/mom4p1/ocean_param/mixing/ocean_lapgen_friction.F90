...
! </INFO>
!
!<NAMELIST NAME="ocean_lapgen_friction_general_nml">
!
...
!  To damp the divergence field.  May be useful for generalized 
!  vertical coordinates to reduce noise. 
!  </DATA> 
...
use mpp_mod,          only: mpp_sum, mpp_pe, mpp_error, mpp_max, mpp_chksum 
...
use ocean_workspace_mod,  only: wrk1, wrk2, wrk1_2d, wrk1_v, wrk1_v2d  
...
character(len=256) :: version=&
     '=>Using: ocean_lapgen_friction.F90 ($Id: ocean_lapgen_friction.F90,v 16.0.2.3.84.1 2009/10/10 00:42:22 nnz Exp $)'
...
subroutine ocean_lapgen_friction_init(Grid, Domain, Time, Ocean_options, d_time, &
  ...
  if(neptune) then 
    write(stdoutunit,'(1x,a)') '==> Note: Computing friction relative to Neptune'
...
subroutine lapgen_friction(Time, Thickness, Adv_vel, Velocity, &
  ...
  if(energy_analysis_step) then 
      ...
      if (id_horz_lap_diss > 0) then 
          do k=1,nk
             ...
             enddo
          enddo
          used = send_data(id_horz_lap_diss, wrk2(isc:iec,jsc:jec,:),&
          Time%model_time, rmask=Grd%umask(isc:iec,jsc:jec,:),       &
          is_in=isc, js_in=jsc, ks_in=1, ie_in=iec, je_in=jec, ke_in=nk)
      endif 
  ...
  if(debug_this_module) then
      ...
      write(stdoutunit,*) 'lapgen friction(1) = ', &
         mpp_chksum(wrk1_v(isc:iec,jsc:jec,:,1)*Grd%umask(isc:iec,jsc:jec,:))
      write(stdoutunit,*) 'lapgen friction(2) = ', &
...
!                    Stephen.Griffies@noaa.gov (7/2007)  
!
!
...
! Coded in mom4 by Stephen.Griffies@noaa.gov 
!
...
subroutine compute_neptune_velocity(Time)
  ...
  id_neptune_u   = register_static_field('ocean_model', 'neptune_u',    &
                   Grd%vel_axes_uv(1:2), 'Zonal velocity from Neptune', &
                   'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  id_neptune_v   = register_static_field('ocean_model', 'neptune_v',         &
                   Grd%vel_axes_uv(1:2), 'Meridional velocity from Neptune', &
                   'm/sec', missing_value=missing_value, range=(/-1.e10,1.e10/))
  id_neptune_psi   = register_static_field('ocean_model', 'neptune_psi', &
                     Grd%tracer_axes(1:2), 'Streamfunction for Neptune', &
                     'm^2/sec', missing_value=missing_value, range=(/-1.e12,1.e12/))
  id_neptune_ft   = register_static_field('ocean_model', 'neptune_ft',              &
                    Grd%tracer_axes(1:2),'Coriolis parameter used for Neptune Psi', &
