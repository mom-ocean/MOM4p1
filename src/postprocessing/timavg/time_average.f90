...
!-----------------------------------------------------------------------
  ! initalize site blksz - for gfdl it's 64k
    blksz = 65536
  ...
  ! create version string (may replace with CVS $Id: time_average.f90,v 16.0.4.1.2.1.2.1 2009/11/03 20:26:51 fms Exp $)
...
!-----------------------------------------------------------------------
 ...
 do ifile = 1, MAX_FILES
    ...
    !--- opening input files ---
     istat = NF90_OPEN (trim(file_names(ifile)), NF90_NOWRITE, ncid_in, blksz)
...
!print *, trim(Vars(ivar)%name),': use_data_wt,do_missval=',Vars(ivar)%use_data_wt,Vars(ivar)%do_missval
   ...
   !---------------- OUTPUT FILE INITIALIZATION -----------------
     ...
     if (ifile == 1) then
       ! create file
         istat = NF90_CREATE (trim(file_name_out), NF90_CLOBBER+NF90_64BIT_OFFSET, ncid_out, blksz)
