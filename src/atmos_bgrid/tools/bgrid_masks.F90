...
!  version number info
   ...
   character(len=128) :: version='$Id: bgrid_masks.F90,v 11.0 2004/09/28 19:07:36 fms Exp $'
      ...
      if (maxres > 1.0001) then
         ...
         if (mpp_pe() == mpp_root_pe()) then
            if (Vgrid%hybrid) then
               write (stdlog(),100) 'eta/hybrid'
            else
               write (stdlog(),100) 'eta'
            endif
         ...
         if (mpp_pe() == mpp_root_pe()) then
            if (Vgrid%hybrid) then
               write (stdlog(),100) 'sigma/hybrid'
            else
               write (stdlog(),100) 'sigma'
            endif
         endif
      endif
