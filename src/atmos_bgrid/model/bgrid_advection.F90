...
!  performance timing info
 ...
 character(len=128) :: version='$Id: bgrid_advection.F90,v 12.0 2005/04/14 15:33:49 fms Exp $'
...
!         defines advection control parameters (stored in advec_control_type)
 ...
 real    :: wt
 integer :: n, m, np, ntrace, unit, ierr, io
...
! write version, namelist info to log file
   if (do_log) then
      ...
      if (mpp_pe() == mpp_root_pe()) write (stdlog(), nml=bgrid_advection_nml)
      do_log = .false.
   endif
