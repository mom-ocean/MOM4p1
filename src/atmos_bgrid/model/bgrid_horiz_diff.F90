...
! set via the namelist or field table
 ...
 character(len=128) :: version='$Id: bgrid_horiz_diff.F90,v 11.0 2004/09/28 19:06:54 fms Exp $'
...
integer :: j
 ...
 integer            :: n, nv, order, ntrace, unit, ierr, io
 real               :: coeff, slope(4)
...
! read namelist
 ...
 5     call close_file (unit)
   ...
   if (do_log) then
      ...
      if (mpp_pe() == mpp_root_pe()) write (stdlog(), nml=bgrid_horiz_diff_nml)
      do_log = .false.
   endif
